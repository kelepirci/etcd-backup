...
def all_procedure(debug_enabled,
    ...
    def v2_backup():
        ...
        p = subprocess.Popen([
            "/usr/local/bin/etcdctl backup --data-dir %s --backup-dir /tmp/%s" % (etcd_v2_datadir, tmp_dir),
    ...
    def clean_temp():
        logger.info('/tmp/%s deleted' % tmp_dir)
        try:
            shutil.rmtree("/tmp/%s/" % tmp_dir)
        except OSError as e:
            logger.error("Error: %s - %s." % (e.filename, e.strerror))
        os.remove("/tmp/%s.zip" % tmp_dir)
        logger.info('/tmp/%s.zip deleted' % tmp_dir)
